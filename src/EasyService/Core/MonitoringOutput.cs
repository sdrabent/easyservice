...
public static class MonitoringOutput
{
    ...
    {
        new("easyservice_service_running", "gauge", "1 = the Windows service is running",
            r => r.Info?.IsRunning == true ? 1 : 0),
        new("easyservice_application_running", "gauge", "1 = the supervised application is running",
            r => r.State?.State == SupervisorState.Running ? 1 : 0),
        new("easyservice_check_status", "gauge", "0 = ok, 1 = warning, 2 = critical, 3 = unknown",
            r => (double)(int)r.Status),
        new("easyservice_uptime_seconds", "gauge", "Uptime of the application since its last start",
            r => r.State?.Uptime?.TotalSeconds ?? 0),
        new("easyservice_restarts_total", "counter", "Restarts since the service was started",
            r => r.State?.RestartCount ?? 0),
        new("easyservice_restarts_1h", "gauge", "Restarts in the last hour",
            r => r.State?.RestartsLastHour ?? 0),
        new("easyservice_restarts_24h", "gauge", "Restarts in the last 24 hours",
            r => r.State?.RestartsLastDay ?? 0),
        new("easyservice_cpu_percent", "gauge", "CPU load of the process tree (100 = all cores busy)",
            r => r.State?.CpuPercent ?? 0),
        new("easyservice_cpu_seconds_total", "counter", "CPU time consumed by the process tree",
            r => r.State?.CpuSecondsTotal ?? 0),
        new("easyservice_memory_bytes", "gauge", "Working set of the process tree",
            r => r.State?.WorkingSetBytes ?? 0),
        new("easyservice_processes", "gauge", "Number of processes in the tree",
            r => r.State?.ProcessCount ?? 0),
        new("easyservice_last_exit_code", "gauge", "Exit code of the last application run",
            r => r.State?.LastExitCode ?? 0),
        new("easyservice_state_age_seconds", "gauge", "Age of the supervisor's last status report",
